...
export default function Home() {
  ...
  const load = React.useCallback(async () => {
    try { const r = await api.listHistory(); setHistory(r.items.slice(0, 5)); } catch {}
  }, []);
  useFocusEffect(React.useCallback(() => { loadPrefs(); load(); }, [load]));

  const onRefresh = async () => { setRefreshing(true); await load(); setRefreshing(false); };
  const greetName = prefs.userName ? `, ${prefs.userName.split(" ")[0]}` : "";
      ...
      <SafeAreaView edges={["top"]} style={{ flex: 1 }}>
        <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}
          ...
          <View style={styles.header}>
            <View>
              <Text style={styles.hi}>Hi there{greetName} 👋</Text>
              <Text style={styles.helper}>How may I help you today?</Text>
            </View>
            <TouchableOpacity testID="profile-shortcut" onPress={() => router.push("/(tabs)/profile")}>
          ...
          <View style={styles.heroCard}>
            ...
            <View style={styles.heroLeft}>
              <Image source={{ uri: "https://images.pexels.com/photos/28408917/pexels-photo-28408917.jpeg" }} style={styles.heroOrb} />
            </View>
            <View style={styles.heroRight}>
              ...
              <TouchableOpacity testID="hero-start-chat-button" style={styles.heroBtn} onPress={() => router.push("/chat")} activeOpacity={0.85}>
          ...
          <View style={styles.actionsGrid}>
            {QUICK_ACTIONS.map((a) => (
              <TouchableOpacity key={a.key} testID={`quick-action-${a.key}`} style={styles.actionCard} activeOpacity={0.85} onPress={() => router.push(a.route as any)}>
          ...
          {history.length === 0 ? (
            ...
            const from = getLanguage(h.source_lang); const to = getLanguage(h.target_lang);
            return (
              <TouchableOpacity key={h.id} testID={`history-card-${h.id}`} style={styles.histCard} activeOpacity={0.85} onPress={() => router.push("/(tabs)/saved")}>
                <View style={styles.histFlags}><Text style={styles.histFlag}>{from.flag}</Text><Text style={styles.histFlag}>{to.flag}</Text></View>
                <View style={{ flex: 1 }}>
...
const styles = StyleSheet.create({
  ...
  heroCard: { height: 160, borderRadius: 24, overflow: "hidden", flexDirection: "row", alignItems: "center", padding: 18, marginBottom: 24 },
  ...
  heroBtn: { flexDirection: "row", alignItems: "center", alignSelf: "flex-start", gap: 6, backgroundColor: "#fff", paddingHorizontal: 14, paddingVertical: 8, borderRadius: 999, marginTop: 12 },
  heroBtnText: { color: "#FF2E93", fontSize: 12, fontWeight: "700" },
  sectionTitle: { color: "#fff", fontSize: 18, fontWeight: "700", marginBottom: 14 },
  actionsGrid: { flexDirection: "row", flexWrap: "wrap", justifyContent: "space-between", gap: 12, marginBottom: 28 },
  actionCard: { width: "47%", backgroundColor: "rgba(255,255,255,0.05)", borderRadius: 22, padding: 18, borderWidth: 1, borderColor: "rgba(255,255,255,0.08)" },
  ...
  emptyCard: { backgroundColor: "rgba(255,255,255,0.04)", borderWidth: 1, borderColor: "rgba(255,255,255,0.08)", borderRadius: 20, padding: 24, alignItems: "center", gap: 12 },
  emptyText: { color: "#9CA3AF", textAlign: "center", fontSize: 13 },
  histCard: { flexDirection: "row", alignItems: "center", gap: 12, padding: 14, backgroundColor: "rgba(255,255,255,0.05)", borderRadius: 18, borderWidth: 1, borderColor: "rgba(255,255,255,0.08)", marginBottom: 10 },
