            ...
            return (
              <View style={styles.card} testID={`saved-card-${item.id}`}>
                <View style={styles.cardHead}>
                  ...
                  <View style={styles.kind}><Ionicons name={item.kind === "voice" ? "mic" : item.kind === "image" ? "camera" : "text"} size={12} color="#C9B8E4" /><Text style={styles.kindText}>{item.kind.toUpperCase()}</Text></View>
                </View>
                ...
                <View style={styles.cardActions}>
                  <TouchableOpacity testID={`fav-${item.id}`} onPress={() => onFav(item.id)} style={styles.actBtn}><Ionicons name={item.favorite ? "star" : "star-outline"} size={18} color={item.favorite ? "#FFD43B" : "#C9B8E4"} /></TouchableOpacity>
                  <TouchableOpacity testID={`del-${item.id}`} onPress={() => onDel(item.id)} style={styles.actBtn}><Feather name="trash-2" size={16} color="#C9B8E4" /></TouchableOpacity>
                </View>
              </View>
            );
          }} />
